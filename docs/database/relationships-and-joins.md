...
> Tài liệu tham khảo về các loại quan hệ (1:1, 1:N, N:M) và các câu lệnh JOIN trong database.
> Tất cả ví dụ sử dụng schema thực tế của dự án demo-vibe.
...
Dự án demo-vibe dùng MongoDB nên không có quan hệ 1:1 thuần túy (thay vào đó embed trực tiếp). Nhưng nếu dùng SQL, ví dụ tách bảng User Profile:
...
-- Bảng Profile tách riêng thông tin cá nhân (1 User = 1 Profile)
...
SELECT * FROM TB_COMM_RFRSH_TKN WHERE USE_ID = '660a1b2c3d4e5f6a7b8c9d0e';
```
...
```sql
SELECT * FROM TB_COMM_BOARD_POST WHERE USE_ID = '660a1b2c3d4e5f6a7b8c9d0e';
```
...
```sql
SELECT * FROM TB_COMM_BOARD_CMNT WHERE POST_ID = '660a1b2c3d4e5f6a7b8c9d0e';
```
...
```sql
SELECT * FROM TB_COMM_ORDR_ITEM WHERE ORDR_ID = '660a1b2c3d4e5f6a7b8c9d0e';
```
...
- Bảng trung gian thường có prefix `TR_` (relation) trong dự án này
...
WHERE mbr.USE_ID = '660a1b2c3d4e5f6a7b8c9d0e';

-- Lấy tất cả member của chat room
SELECT u.*
FROM TB_COMM_USER u
JOIN TR_COMM_CHAT_ROOM_MBR mbr ON u._id = mbr.USE_ID
WHERE mbr.CHAT_ROOM_ID = '660a1b2c3d4e5f6a7b8c9d0e';
```
...
WHERE POST_ID = '660a1b2c3d4e5f6a7b8c9d0e';

-- Kiểm tra user đã like bài viết chưa
SELECT COUNT(*) > 0 AS is_liked
FROM TR_COMM_BOARD_LIKE
WHERE POST_ID = '660a...' AND USE_ID = '661b...';
```
...
WHERE o.BYR_ID = '660a1b2c3d4e5f6a7b8c9d0e'
...
WHERE p.DEL_YN = 'N'
  AND p.SLLR_ID = '660a1b2c3d4e5f6a7b8c9d0e'
...
> **Thực tế:** RIGHT JOIN ít dùng vì luôn có thể viết lại thành LEFT JOIN bằng cách đổi thứ tự bảng. Ví dụ trên tương đương:
...
**Lưu ý:** MySQL không hỗ trợ `FULL OUTER JOIN`. Thay thế bằng `UNION`:

```sql
-- MySQL workaround
SELECT SLLR_ID, product_count, order_item_count FROM (
    SELECT p.SLLR_ID, COUNT(p._id) AS product_count, 0 AS order_item_count
    FROM TB_PROD_PRD p GROUP BY p.SLLR_ID
    UNION ALL
    SELECT oi.SLLR_ID, 0, COUNT(oi._id)
    FROM TB_COMM_ORDR_ITEM oi GROUP BY oi.SLLR_ID
) combined
GROUP BY SLLR_ID;
```
...
## 3. So sánh tổng hợp các loại JOIN
...
INNER JOIN TB_COMM_USER u ON p.USE_ID = u._id
WHERE p.DEL_YN = 'N'
  AND p.RGST_DT >= DATE_SUB(NOW(), INTERVAL 7 DAY)
...
LEFT JOIN TL_COMM_LGN_LOG l
    ON u._id = l.USE_ID
    AND l.LGN_DT >= DATE_SUB(NOW(), INTERVAL 30 DAY)
